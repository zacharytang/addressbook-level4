...
=== Adding/Deleting a Photo for a person : `photo` _(Coming in v1.4)_
____
...
* Ensure that format of the path to the photo file is correct.
...
=== Locating persons by filters : `find` _(Coming in v2.0)_
____
...
Format: `find t/Tag p/PHONE e/EMAIL b/BIRTHDAY MONTH...` +
...
* The result will be ordered in terms of ascending order of their birthday dates.
* Example: Person with birthday on October 10 is returned after someone with October 2.
****
...
****
* The tag that you want to replace has to already exist in the address book .
...
=== Deleting a person, multiple people or a tag: `delete`
==== Deleting a person or multiple persons _(Added since v1.3)_
____
...
____
Deletes the contacts who you have not viewed/edited/listed by the 'edit', 'photo' or 'find' command
...
* `delete old/2` +
Deletes all the contacts which you have not viewed/edited/listed by the 'edit', 'photo' or 'find' command for the past 2 months.
...
****
* Selects the person and loads the Google search page for the person at the specified `INDEX`.
****
...
=== Open a Google Maps view of a selected person's address : `gmaps` _(Added since v1.3)_
____
_You are meeting your group mate at his house. But where is his house on the map?_
____
For a person specified by an index, opens a Google Maps view of the person's address location. +
...
* `password 123456789` +
Adds 123456789 as a password to UniFy.
* `password` +
To clear the password set in UniFy.
