...
* `INDEX` refers to the targetIndex number shown in the last generated listing of people.
...
____
By specifying an targetIndex or multiple indexes, you can delete specified people in the most recent listing from the address book. +
Format: +
...
* If no targetIndex is specified, displays the timetable representing all the persons currently listed.
...
____
For a person specified by an targetIndex, opens a Google Maps view of the person's address. +
