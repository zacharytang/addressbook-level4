...
==== Locating persons by birthday _(Coming in v1.3)

Finds persons whose birthday is in the given month. +
Format: `find b/MONTH` +

****
* MONTH should be inputted by numbers. e.g `12` searches for December babies
* Single digit months needs a 0 in front. e.g `02` searches for Febuarary babies
* Only the birthday month is searched.
* Persons matching the correct birthday month will be returned in ascending order of their birthday dates.
****

Examples:

* `find b/12` +
Returns any person having birthday in December

* `find b/05` +
Returns any person having brithday in May
...
* The command parser accepts duplicated indexes.
****
...
* *Add* : `add n/NAME g/GENDER m/MATRIC_NO p/PHONE_NUMBER e/EMAIL a/ADDRESS [b/BIRTHDAY] [t/TAG]...` +
e.g. `add n/James Ho g/Male m/A0162525k p/22224444 e/jamesho@example.com a/123, Clementi Rd, 1234665 b/26021995 t/friend t/colleague`
* *Clear* : `clear`
