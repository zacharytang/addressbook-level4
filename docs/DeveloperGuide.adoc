...
UniFy is a command-line application which provides a way to store students' contact information. It provides powerful functions and user-friendly interface for NUS students, and it allows them to efficiently manage their contact lists. This developer guide provides a brief introduction to the basic information about this application, helping you get started with the contribution work.
...
Users are able to store timetables by supplying a shortened NUSMods url when adding a person.
...
----
Map<String, Object> mappedJson = mapper.readValue(url, HashMap.class); // read JSON as map
ArrayList<HashMap<String, String>> lessonInfo = mappedJson.get("Timetable"); // retrieve lesson data
...
[NOTE]
XYZComponent refers to all other components that `Person` is composed of, the class diagram is not complete.
...
The need for this came about from the User Story in a which a user would like to delete a tag that was no longer relevant or is outdated.
...
==== [DeleteCommandParser] Detection of the Deletion Type
...
The preamble for `delete INDEX` is a *digit* while the preamble for `delete t/TAG` is a *blank*.
...
==== [DeleteCommand] Overloading the DeleteCommand constructor
...
==== [DeleteCommand] Logic Execution depending on which attributes are present.
...
Not shown in Figure B, `listTags` are checked against `listOfExistingTags`, i.e. all tags
...
_Figure A. Typical command execution_

image::DeleteCommandForTagSequenceDiagram2.png[width="800"]
_Figure B. The tags parsed are put into an ArrayList and iterated through for deletion_

image::DeleteCommandForTagSequenceDiagram3.png[width="800"]
_Figure C. How each tag is removed from the Address Book and each Person's list of tags_
...
Augmenting the command via overriding constructors, adding new attributes
and modifying the parse may seem too convoluted a solution.
Users who are used to the previous version of the command may not appreciate the new change
If bugs arise from this implementation, these users will be very unhappy because this is a change
to the command that to them may be uncalled for. +
...
**Cons:** `deletetag t/tag` is not as inuitive a command line especially to new or casual users.
...
**Reason:** We have decided that the Model API deal with objects in singular amounts, i.e. deletePerson, addPerson, deleteTag,
as opposed to collections of objects, i.e. deletePersons, deleteTags.
In order to maintain consistency within the API, the first choice was chosen.
...
It is different from adding the tags using 'add' command as it is used to store unique information of contact.
The reason why we need to implement Remark Command can be found in the User Stories: the user wants to save more details about the contacts.

Generally, the implementation of this command is similar to 'edit' command.
...
1. AddressBookParser
2. PersonListCard, PersonCardHandle
3. XmlAdaptedPerson
4. Person
5. EditCommand
6. AddCommand
...
Instead of typing “remark” in the command box, one can also type “rm” to represent “remark” as it will also be parsed by the parser.
The alias is added to the RemarkCommand Class.
The added remark will be displayed on the last line of the person card.
If a person is newly added to the personList by Add Command, its remark field will have an empty string at first. Also, in the EditCommand class, a new attribute updatedRemark is added to the person, and it is independent from editPersonDescriptor.

==== [BasicStep]Implement this command
...
==== [Detail Implementation]Codes and Diagrams
...
_Figure A. High-level sequence diagram

The RemarkCommand extends Undoale Command, which is an abstract subclass of abstract class Command, so the user can also undo the added remark.
The class inheritance diagram is shown below:

image::RemarkCommandClassInheritanceDiagram.png[width="800"]
_Figure B. Class Inheritance Diagram
...
_Figure C. Logic Component Sequence Diagram
...
_Figure D. Component Class Diagram
...
Also, the font-size is bigger
**Cons:** It does not the consistent format with other fields in the person card.
If the remark is too long, it will be poorly displayed.
...
image::BirthdayModelComponentClassDiagram.png[width="500"]
...
public static boolean isValidBirthday(String test) {
    int month = Integer.valueOf(test.subString(2, 4));
    int date = Integer.valueOf(ttest.subString(0, 2));
    int year = Integer.valueOf(test.subString(4));

    if (!test.matches(BIRTHDAY_VALIDATION_REGEX) {
        // Number of digits input is not 8
        return false;
    } else if (1900 > year || year > 2017){
        return false;
    } else {
        //Check for months with 30 days, with 31 days and Feburary
        if (month == 2)
            return date <= 29;
...
**Cons:** Inconsistent with the fomat in person card
...
....
To be suggested a command based on the detection of a misspelling of a command (e.g. find is suggested when fnid is spelled)
....
