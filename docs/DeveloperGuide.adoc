...
public DeleteCommand parse(String args) throws ParseException {
    ...
    if (preamble.equals("")) {
        ...
        // parsing of the targetIndex
        return new DeleteCommand(targetIndex); # <2>
    }
    // ...
}
----
...
<2> a `new DeleteCommand(targetIndex)` is returned when deleting a person.
...
|`* * *` |user |select a person from a list by the targetIndex  |easily select a person from a list I have just seen
...
1.  User requests to list persons
2.  AddressBook shows a list of persons
3.  User requests to edit the details of the person identified by the targetIndex number in the listing
