...
**Pros:** Does not lose any undone/redone functionality as it is now part of the default behaviour. Classes that deal with `Command` will not know that `executeUndoableCommand()` exist. +
...
**Pros:** Uses less memory (e.g. for `delete`, just save the person being deleted). +
...
**Alternative 2:** Includes all the commands. +
...
**Cons:** Require dealing with commands that have already been undone: We must remember to skip these commands. Violates Single Responsibility Principle and Separation of Concerns as `HistoryManager` now needs to do two different things. +
// end::undoredo[]
...
**Alternative 2:** Uses a 3D array to represent the entire timetable +
...
**Alternative 3:** Stores individual lesson timings and information. +
**Pros:** Building the timetable is not required +
**Cons:** Inefficient queries if a timing has a lesson, needs to iterate through every lesson stored.

---

**Aspect:** Uses Shortened URLs versus full-length URLs +
...
**Alternative 2:** Accepts both shortened and full-length NUSMods URLs +
...
**Alternative 1 (current choice):** Augments the existing delete command +
...
**Alternative 2:** Creates a new command `deletetag` +
...
**Alternative 1 (current choice):** On the `Logic` level, iterates through an array of tags and invoke a Model method `deleteTag(Tag tag)` on each tag.  +
**Pros:** Maintains consistency with the Model API that deals with objects in singular amounts. +
**Cons:** Requires a loop to delete the tags. +
**Alternative 2:** On the `Logic` level, invokes a method `deleteTags(tagSet)` on a Set<Tag>. Implement `deleteTags(Set<Tag> tagSet)` in the Model component. +
...
**Alternative 1(current choice):** Displays the remark of the specified contact at the end of the personCard. +
**Pros:** Has the consistent format with other fields in the person card. +
**Cons:** Does not highlight the remark so that the user might hardly notice the additional remark information. +
**Alternative 2:** Displays the Remark next to the name. +
**Pros:** Placing the remark in this prominent place better reminds the user of the added remark information.
Also, the font-size is larger that is easier to read. +
**Cons:** Does not have the consistent formatting with other fields in the person card.
If the remark is too long, it will be poorly displayed as well.

**Aspects:** The command nature of Remark +
**Alternative 1(current choice):** Uses a RemarkCommand to add remarks to a contact. +
...
**Alternative 2:** Uses AddCommand and EditCommand for adding and editing of the remark. +
...
Users are able to store birthdays by inputting in the format of DDMMYYYY when adding a person.
...
**Alternative 1 (current choice):** Displays the birthday of the specified contact at the end of the personCard. +
**Pros:** Has the consistent format with other fields in the person card. +
**Cons:** Difficult to recognise the number displayed is birthday. +
**Alternative 2:** Displays the birthday with "Birthday:" in front. +
