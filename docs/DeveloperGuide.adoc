...
:repoURL: https://github.com/CS2103T-W09B1/addressbook-level4/tree/master
...
|`* * *` |user who has many friends |view different groups of my friends by their tags |manage my contacts well

|`* * *` |user |view an link:#attribute[attribute] called ‘gender’ |see more details of my friends
...
|`* * *` |user |see the total number of contacts in my address book | have a sense of the size of the address book
...
|`* * *` |user |be informed who I deleted |confirm if I’ve deleted the correct person
...
|`* *` |impatient user |type my commands in shorthand form | execute command quickly and be more productive

|`* *` |user | see the tags in different colors |identify what tags and contact i have faster

|`* *` |user | be link:#suggested[suggested] a correct command if I type a command incorrectly | not need to always check search to see the list of commands

|`* *` |user | remove a specific tag that exists in some/all my contacts | remove a tag group

|`* *` |user | edit a specific tag’s name for all contacts with this tag | update the all the tag name easily
...
|`* *` |user |see which contact live near me | find out who I can conveniently meet nearby

|`* *` |careful user |extract all the contact information into a txt/doc file on desktop |

|`* *` |user who has a limited disk space |delete the contacts who I haven’t viewed for a certain time |

|`* *` |user |be able to see all the existing tag icons in the UI |click one of these tag icons and see all the contacts under this tag

|`* *` |user who likes to celebrate my friends’ birthdays |view different groups of my friends by the month| better plan their celebrations
...
|`*` |office worker |who needs to liaise with different companies |save my business contacts with their occupation, company name and company address easily find the required information whenever I contact them

|`*` | user who likes different styles | change the theme(color) of the UI |

|`*` | short-sighted user | change the font size of the Address Book |

|`*` |user who is outdoors |have a  link:#bright-coloured-mode[bright coloured mode] in the address book | application will be more visible on the screen outdoors

|`*` |student |save my friends’ timetables |know when my friends are free to meet up

|`*` |user with many friends |combine my friends’ timetables by selecting the contacts one by one or by tags| know when is the common time slot for everyone to meet up
