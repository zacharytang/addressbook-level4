...
.. When `TimetableDisplay` component is created, `TimetableDisplay:fillInitialGrid()` first populates the timetable with empty slots.
.. After which, each `Timetable` is added to the grid by `TimetableDisplay:fillSingleTimetable()`.
...
**Alternative 2:** Only store timetable URLs, and query each time app is launched. If no internet, start with empty timetable +
