...
Upon execution of a `select` or `whenfree` command, the following happens:
...
. The event is then handled by the `InfoPanel`, which brings the `TimetableDisplay` component to the front for viewing
